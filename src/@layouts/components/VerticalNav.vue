...
// ดึงบทบาทของผู้ใช้จากคุกกี้
const userDataCookie = useCookie('userData')
const userRole = userDataCookie.value.role

// ฟังก์ชันกรอง navItems ตามบทบาทของผู้ใช้

const filterNavItemsByRole = (navItems, role) => {
  return navItems.filter(item => {
    if (Array.isArray(item.roles)) {
      item.roles = item.roles.filter(role => role == userRole);
    }
    if (item.roles && item.roles != userRole) {
      return false
    }
    if (item.children) {
...
// กรอง nav items ตามบทบาทของผู้ใช้
const filteredNavItems = filterNavItemsByRole(props.navItems, userRole)
  ...
  <Component :is="props.tag" ref="refNav" class="layout-vertical-nav" :class="[
    ...
    <div class="nav-header">
      <slot name="nav-header">
        <RouterLink to="/dashboard" class="app-logo app-title-wrapper">
